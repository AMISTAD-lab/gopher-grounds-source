...
def create_fof(func: str):
    '''
    returns a map of { frequency, fof } for all frequencies generated by a given
    function using the data from the database
    '''
    cursor = client.cursor()

    cursor.execute('\
        SELECT t_freq, COUNT(t_freq) FROM (\
            SELECT SUM(frequency) t_freq FROM frequencies \
            WHERE [function]=:function GROUP BY trap \
        ) GROUP BY t_freq; \
    ', { 'function': func })
...
def populate_fof(func: str, output=None):
    '''
    takes in a generative function.
    calls the create_fof function to get a map from freq -> fof for that function.
    adds the data from this map to the database's fof table.

    optional: if output is passed a string, the function will write the data
    the csv given by output
    '''
    cursor = client.cursor()

    if not setup.exists(FOF_TABLE, 'table'):
        cursor.execute(FOF_SCHEMA)

    fof_map = create_fof(func)

    for freq in fof_map:
        cursor.execute(
            f'INSERT INTO {FOF_TABLE} VALUES (?, ?, ?) \
            ON CONFLICT DO NOTHING;',
            [func, freq, fof_map[freq]]
        )

    if output:
        with open(output, 'w+', newline='') as out:
            writer = csv.writer(out)
            writer.writerow(constants.fofHeaders)

            for freq in fof_map:
                writer.writerow([freq, fof_map[freq]])
